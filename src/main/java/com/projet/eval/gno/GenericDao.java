...
public T[] creerLstObjectsT(T tobj,Connection connection,String sqlprepare,Object[] valuecondition,int offset,int limit)throws Exception{
  String query=sqlprepare+" offset ? limit ? ";
  int taille=2;
  if(valuecondition!=null){ taille=taille+valuecondition.length; }
  Object[] objects=new Object[taille];//ajouter 2 pour les objets offset et limit
  if(taille>2){
    for(int i=0;i<valuecondition.length;i++){
      objects[i]=valuecondition[i];
    }
  }
  objects[taille-2]=offset;
  objects[taille-1]=limit;
  List<HashMap<String,String>> lstValueField=valueSelectToListHashMap(query, objects, connection);
  T[] lstobj=creerLstObjectsT(tobj, lstValueField);
  return lstobj;
}

public T[] creerLstObjectsT(T tobj,Connection connection,String sqlprepare,String[] namefieldcondition)throws Exception{
  if(namefieldcondition!=null){
    Object[] objs=new Object[namefieldcondition.length];
    Field ftemp=null;
    for(int i=0;i<namefieldcondition.length;i++){
      ftemp=tobj.getClass().getField(namefieldcondition[i]);
      ftemp.setAccessible(true);
      objs[i]=ftemp.get(tobj);
      ftemp.setAccessible(false);
    }
    return creerLstObjectsT(tobj, connection, sqlprepare, objs);
  }
  return creerLstObjectsT(tobj, connection, sqlprepare, null);
}
//-----------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------------------save/Insert
  private T save(Object object,Connection connection,String nameprimaryKey)throws Exception{
    String siPkNull="##$$##$$;;";
    ...
    for(int i=0;i<fields.length;i++){
      ...
      if(column!=null ){ //si il n'est pas le pk et qu'il existe comme column dans la table qui lui convient
        if(column.compareToIgnoreCase(nameprimaryKey)!=0){
          ...
          returning=returning+" "+nameprimaryKey+",";
        }
      }
    }
    ...
    requet=requet+")"+values+") "+returning;
    //System.out.println("before execut : "+requet); 
    //this.executeNoSelect(connection, requet, listObj.toArray());
    T[] tobj= this.creerLstObjectsT((T)object, connection, requet, listObj.toArray());
    if(tobj==null){return null;}
    else if(tobj.length>0){ return tobj[0]; }
    else{return null; }
  }

  public T save(Object object,Connection connection)throws Exception{
    return save(object, connection,getInfoDbNameFieldAnnoteByAnnotation((T)object,Id.class));
  }
//----------------------------------------------------------------------------------------------------------update
...
//---------------------------------------------------------------------------------------------------------------find All 
  ...
  public T[] findAllT(T object,Connection connection,int offset,int limit)throws Exception{
    String query="select * from "+object.getClass().getSimpleName()+" offset ? limit ?";
    T[] objs=this.creerLstObjectsT(object, connection, query, new Object[]{offset,limit});
    return objs;
  }

// offset 0 limit 4;
//   public Object insertreturn(Connection connection,String sqlprepare,Object[] valueObjs)throws Exception {
//     // Requête SQL d'insertion avec la clause RETURNING
//     String sql = "INSERT INTO client (nom) VALUES (?) RETURNING idclient, nom";
//     List<HashMap<String,String>> lsthashMap= valueSelectToListHashMap(sqlprepare,valueObjs,connection);
//     creerLstObjects(lsthashMap, connection, sqlprepare, valueObjs)
//     try (PreparedStatement statement = connection.prepareStatement(sql)) {
//         // Définir les valeurs des paramètres de la requête
//         setStatement(statement, valueObjs);
//         ResultSet resultSet = statement.executeQuery();
//         // Vérifier s'il y a un résultat
//         if (resultSet.next()) {
//             // Extraire les valeurs de la ligne insérée
...
//     // Retourner null si l'insertion a échoué
//     return null;
// }
